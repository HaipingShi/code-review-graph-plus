"""CLI entry point for code-review-graph-plus.

Usage:
    code-review-graph-plus install
    code-review-graph-plus init
    code-review-graph-plus build [--base BASE]
    code-review-graph-plus update [--base BASE]
    code-review-graph-plus watch
    code-review-graph-plus status
    code-review-graph-plus serve
    code-review-graph-plus visualize
    code-review-graph-plus wiki
    code-review-graph-plus detect-changes [--base BASE] [--brief]
    code-review-graph-plus register <path> [--alias name]
    code-review-graph-plus unregister <path_or_alias>
...
if sys.version_info < (3, 10):
    print("code-review-graph-plus requires Python 3.10 or higher.")
...
def _get_version() -> str:
    ...
    try:
        return pkg_version("code-review-graph-plus")
...
def _print_banner() -> None:
    ...
    print(f"""
{c}  ●──●──●{r}
{c}  │╲ │ ╱│{r}       {b}code-review-graph-plus{r}  {d}v{version}{r}
{c}  ●──{y}◆{c}──●{r}
...
{c}  ●──●──●{r}       {d}smarter code reviews{r}
  ...
  {d}Run{r} {b}code-review-graph-plus <command> --help{r} {d}for details{r}
""")
...
def _handle_init(args: argparse.Namespace) -> None:
    ...
    print()
    print("Next steps:")
    print("  1. code-review-graph-plus build    # build the knowledge graph")
...
def main() -> None:
    ...
    if args.version:
        print(f"code-review-graph-plus {_get_version()}")
        return
    ...
    if args.command == "eval":
        ...
        if getattr(args, "report", False):
            ...
            print("Run 'code-review-graph-plus eval --report' to generate tables.")
        return
    ...
    if args.command in ("register", "unregister", "repos"):
        ...
        if args.command == "register":
            ...
            if not repos:
                ...
                print("Use: code-review-graph-plus register <path> [--alias name]")
            else:
                for entry in repos:
        ...
        if args.command == "build":
            ...
            if stored_branch and current_branch and stored_branch != current_branch:
                print(
                    ...
                    f"Run 'code-review-graph-plus build' to rebuild."
                )
